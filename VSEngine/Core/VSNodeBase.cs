...
namespace Core.VSEngine
{
...
#endif
    {
        ...
        protected T? Resolve<T>(string portName)
        {
            object? resolvedValue = ExecutionControl.ResolveValue(portName, this);
            if(resolvedValue == null)
            {
                Debug.LogWarning($"Value resolved for port {portName} is null");
                return default;
            }
            else
            {
        ...
        protected bool ResolveMultiValue<T>(string portName, int index, out T? result)
        {
            ...
            object? value = ExecutionControl.ResolveMultiValue(portName, index, this);
            if(value != null)
            {
                result = (T)value;
                return true;
        ...
        protected List<T>? ResolveMultiValue<T>(string portName)
        {
            T? currentValue;
